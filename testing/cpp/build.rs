...
fn main() {
  ...
  println!("build/buildlibs/{}/libs", target);

  // Import bindings from WPI libraries
  println!("cargo:rustc-link-search={}", PathBuf::from(format!("build/buildlibs/{}/libs", target)).canonicalize().unwrap().to_str().unwrap());
  ...
  let bindings = bindgen::Builder::default()
    .header("HALWrapper.h")
    .derive_default(true)
    .clang_arg(format!("-Ibuild/buildlibs/{}/headers", target))
